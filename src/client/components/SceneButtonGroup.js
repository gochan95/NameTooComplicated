...
export default class SceneButtonGroup extends Component {
  ...
  renderSceneButton = () => {
    ...
    var croppedScenes = scenes.filter(i => i != this.props.SceneStore.currentScene);
    return (
      croppedScenes.map(i => <UnderlineButton key={i} text={i} onClick={this.loadScene.bind(this, i)}/>)
    );
  }
  ...
  render() {
    ...
    return (
      <div className="scene-button-group-container">
        ...
        {currentScene && <UnderlineButton text={this.state.showSceneText} onClick={this.showMore}/>}
      </div>
    );
  }
}
